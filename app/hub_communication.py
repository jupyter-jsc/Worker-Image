...
def token(app_logger, uuidcode, app_hub_url_proxy_route, app_hub_url_token, jhubtoken, accesstoken, expire, username, server_name):
    ...
    app_logger.trace("{} - Access_token: {} , expire: {}".format(uuidcode, accesstoken, expire))
    hub_header = {"Authorization": "token {}".format(jhubtoken),
                  "uuidcode": uuidcode,
                  "Intern-Authorization": get_jhubtoken()}
    ...
    try:
        ...
        if server_name != '':
            url = url + '/' + server_name
        app_logger.trace("{} - Update JupyterHub Token: {} {} {}".format(uuidcode, url, hub_header, hub_json))
...
def status(app_logger, uuidcode, app_hub_url_proxy_route, app_hub_url_status, jhubtoken, status, username, server_name=''):
    app_logger.debug("{} - Send status to JupyterHub: {}".format(uuidcode, status))
    hub_header = {"Authorization": "token {}".format(jhubtoken),
                  "uuidcode": uuidcode,
                  "Intern-Authorization": get_jhubtoken()}
    hub_json = {
    ...
    try:
        ...
        if server_name != '':
            url = url + '/' + server_name
        app_logger.trace("{} - Update JupyterHub Status: {} {} {}".format(uuidcode, url, hub_header, hub_json))
        with closing(requests.post(url,
                                   headers = hub_header,
                                   json = hub_json,
                                   verify = False,
...
def cancel(app_logger, uuidcode, app_hub_url_proxy_route, app_hub_url_cancel, jhubtoken, errormsg, username, server_name=''):
    ...
    try:
        ...
        if server_name != '':
            url = url + '/' + server_name
        app_logger.trace("{} - Cancel JupyterHub: {} {}".format(uuidcode, url, hub_header))
        with closing(requests.delete(url,
                                     headers = hub_header, 
                                     verify = False,
                                     timeout = 1800)) as r:
