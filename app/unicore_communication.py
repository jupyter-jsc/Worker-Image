...
def request(app_logger, uuidcode, method, method_args):
    app_logger.debug("{} - UNICORE/X communication. {} {}".format(uuidcode, method_args.get('url', '<no url>'), method))
    app_logger.trace("{} - UNICORE/X communication. Method_args: {}".format(uuidcode, method_args))
    if method == "GET":
        with closing(requests.get(method_args.get("url"),
            ...
            if r.status_code != 432 and r.status_code != 500: # 432 -> SecuritySession expired. 500 -> Internal Server Error, maybe GPFS made bs?
                if 'return_content' in method_args:
                    ...
                    app_logger.trace("{} - UNICORE/X communication response Content: {}".format(uuidcode, r.content.decode("utf-8")))
                    app_logger.trace("{} - UNICORE/X communication response Headers: {}".format(uuidcode, r.headers))
                    return r.content.decode("utf-8"), r.status_code, r.headers
                else:
                    app_logger.trace("{} - UNICORE/X communication response Status code: {}".format(uuidcode, r.status_code))
                    app_logger.trace("{} - UNICORE/X communication response Content: {}".format(uuidcode, r.text))
                    app_logger.trace("{} - UNICORE/X communication response Headers: {}".format(uuidcode, r.headers))
                    return r.text, r.status_code, r.headers
        app_logger.debug("{} - Session expired. Use accesstoken instead".format(uuidcode))
        try:
        ...
        with closing(requests.get(method_args.get("url"),
            ...
            if 'return_content' in method_args:
                ...
                app_logger.trace("{} - UNICORE/X communication response Headers: {}".format(uuidcode, r2.headers))
                return r2.content.decode("utf-8"), r2.status_code, r2.headers
            else:
                app_logger.trace("{} - UNICORE/X communication response Status code: {}".format(uuidcode, r2.status_code))
                app_logger.trace("{} - UNICORE/X communication response Content: {}".format(uuidcode, r2.text))
                app_logger.trace("{} - UNICORE/X communication response Headers: {}".format(uuidcode, r2.headers))
