...
from app.dashboards_utils import stop_job
from app import unicore_utils, utils_file_loads, unicore_communication,\
    hub_communication, tunnel_utils, dashboards_threads, orchestrator_communication
...
class Dashboards(Resource):
            ...
            if properties_json.get('status') in ['QUEUED', 'READY', 'RUNNING', 'STAGINGIN']:
                if '.end' in children or '/.end' in children:
                    ...
                    if request.headers.get('pollspawner', 'false').lower() == 'true':
                        ...
                        t = Thread(target=dashboards_threads.get,
                                   args=(app.log,
                                         uuidcode,
                                         request_headers,
                                         unicore_header,
    ...
    def post(self):
        try:
            ...
            try:
                unicore_input = unicore_utils.create_inputs_dashboards(app.log,
                                                                       uuidcode,
                                                                       request.json,
            ...
            unicore_json = unicore_utils.create_unicore8_job_dashboard(app.log,
                                                                       uuidcode,
                                                                       request.json,
                                                                       request.headers.get('Project'),
                                                                       unicore_input,
